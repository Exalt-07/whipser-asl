...
class WhiStressTrainer(Seq2SeqTrainer):
    ...
    def save_final_model(self, output_dir=None, training_args=None):
        ...
        if output_dir is not None:
            ...
            torch.save(
                self.model.encoder_projector.state_dict(),
                os.path.join(output_dir, "encoder_projector.pt")
            )
            # torch.save(
            #     {
            #         "encoder_weights": self.model.encoder_layer_weights.data,
            #         "decoder_weights": self.model.decoder_layer_weights.data
            #     },
            #     os.path.join(output_dir, "layer_weights.pt")
            # )
    ...
    def evaluate_at_word_level(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval", dataset_name=''):
        ...
        for batch in tqdm(eval_dataloader):
            ...
            batch_samples = torch.where(torch.tensor(eval_dataset['sentence_index']) == batch['sentence_index'].unsqueeze(1).cpu())[1].numpy()
