...
class WhiStress(PreTrainedModel):
    ...
    def __init__(
        ...
        layer_for_head: Optional[int] = None, # Argument kept for backward compatibility
        ...
        # --- ARCHITECTURAL CHANGE: Learnable Layer Weights ---
        # Whisper returns a tuple of states: (embeddings, layer1, ... layerN)
        # So we need config.encoder_layers + 1 weights.
        num_tensors = config.encoder_layers + 1
        
        # Initialize weights to 1.0 (uniform attention distribution initially)
        self.encoder_layer_weights = nn.Parameter(torch.ones(num_tensors))
        self.decoder_layer_weights = nn.Parameter(torch.ones(num_tensors))
        
        # Dummy value to prevent crashes in legacy code
        self.layer_for_head = -1 
        # -----------------------------------------------------
    ...
    def to(self, device: str = ("cuda" if torch.cuda.is_available() else "cpu")):
        ...
        # Parameters (layer_weights) move automatically with super().to()
        super().to(device)
        return self
    ...
    def load_model(self, save_dir=None, device="cuda" if torch.cuda.is_available() else "cpu"):
        ...
        if save_dir is not None:
            ...
            # Try to load the learnable weights if they exist
            weights_path = os.path.join(save_dir, "layer_weights.pt")
            if os.path.exists(weights_path):
                print("Loading learnable layer weights...")
                weights_dict = torch.load(weights_path, map_location=torch.device(device), weights_only=False)
                self.encoder_layer_weights.data = weights_dict["encoder_weights"]
                self.decoder_layer_weights.data = weights_dict["decoder_weights"]
            
            # Legacy: read metadata.json just to prevent crashes if code expects it
            meta_path = os.path.join(save_dir, "metadata.json")
            if os.path.exists(meta_path):
                with open(meta_path, "r") as f:
                    _ = json.load(f)

    # --- OOM FIX: Iterative Weighted Sum ---
    def _compute_weighted_sum(self, hidden_states_tuple, weights):
        """
        Computes the weighted sum of hidden states iteratively.
        Using loop accumulation prevents creating a massive temporary tensor
        (which torch.stack does), solving the OOM error.
        """
        # Normalize weights (Softmax ensures they sum to 1)
        norm_weights = F.softmax(weights, dim=0)
        
        # Initialize sum with the first layer * its weight
        # This allocates memory for only ONE layer result.
        weighted_sum = hidden_states_tuple[0] * norm_weights[0]
        
        # Iteratively add the rest
        for i in range(1, len(hidden_states_tuple)):
            weighted_sum += hidden_states_tuple[i] * norm_weights[i]
    ...
    def train(self, mode: Optional[bool] = True):
        ...
        # Ensure new layer weights are trainable
        self.encoder_layer_weights.requires_grad = True
        self.decoder_layer_weights.requires_grad = True
    ...
    def forward(
        ...
        backbone_outputs = self.whisper_model(
            ...
            output_hidden_states=True, # Critical: Must return all layers
            labels=whisper_labels,
        )

        # --- ARCHITECTURAL CHANGE: Use Weighted Sum ---
        decoder_weighted_states = self._compute_weighted_sum(
            backbone_outputs.decoder_hidden_states, 
            self.decoder_layer_weights
        ).to(device)

        encoder_weighted_states = self._compute_weighted_sum(
            backbone_outputs.encoder_hidden_states, 
            self.encoder_layer_weights
        ).to(device)
        # ----------------------------------------------
        ...
        additional_decoder_block_outputs = self.additional_decoder_block(
            hidden_states=decoder_weighted_states,
            encoder_hidden_states=encoder_weighted_states,
        )
